...
def process():
    # Мы читаем файл через кодировку latin-1, чтобы просто достать ключи,
    # игнорируя то, что значения там битые.
    with open('C:/Users/Lecoo/projects/GnomoriaTranslator/Gnomoria_raw_catch.json', 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read()
        # Пытаемся распарсить как есть, если не выйдет - будем регулярками
        try:
            data = json.loads(content)
        except:
            import re
            # Достаем все ключи в кавычках перед двоеточием
            keys = re.findall(r'\"(.*?)\"\s*:', content)
            data = {k: k for k in keys}

    t_map = get_full_mapping()
    sorted_keys = sorted(t_map.keys(), key=len, reverse=True)
    
    final_dict = {}
    for original_key in data.keys():
        # Если ключ пустой или слишком короткий
        if not original_key.strip(): continue
        
        # Начинаем с ключа как значения
        translated_value = original_key
        
        # Специальные случаи
        if original_key == "v1.0":
            translated_value = "v0.0.2 | memasevich 2026"
        else:
            # Применяем массовую замену по словарю
            for eng in sorted_keys:
                if eng in translated_value.lower():
                    rus = t_map[eng]
                    # Заменяем с сохранением регистра для первой буквы
                    translated_value = translated_value.replace(eng, rus)
                    translated_value = translated_value.replace(eng.capitalize(), rus.capitalize())

        final_dict[original_key] = translated_value

    # Сохраняем ИДЕАЛЬНО чистый JSON
    with open('C:/Users/Lecoo/projects/GnomoriaTranslator/Gnomoria_ULTRA_CLEAN.json', 'w', encoding='utf-8') as f:
        json.dump(final_dict, f, ensure_ascii=False, indent=2)

    print(f"Final reconstruction complete: {len(final_dict)} entries.")
