...
**Разработчик проекта:** memasevich
...
- разработан автономный графический установщик с фирменной заставкой и двуязычным описанием;
...
## Ключевые особенности версии 0.5:
...
2. Перейдите в раздел [**Релизы (Releases)**](https://github.com/memasevich/Gnomoria-Russian-Translation/releases) и скачайте файл **`GnomoriaRussianTranslationInstaller.exe`**.
3. Запустите установщик. Он автоматически обнаружит путь к установленной игре в Steam (при необходимости путь можно указать вручную через кнопку «Обзор»).
4. Нажмите кнопку **«Установить перевод»**. Установщик самостоятельно:
   * создаст безопасную резервную копию `Gnomoria.exe.backup`;
   * развернет русификатор, актуальный словарь и русские шрифты;
...
> **Удаление/Откат:** при необходимости в установщике доступна кнопка **«Восстановить оригинал»**, которая возвращает исходный `Gnomoria.exe` из бэкапа в один клик.

---

### Способ 2. Ручная установка из архива

1. Убедитесь, что игра Gnomoria закрыта.
2. Скачайте архив **`Gnomoria_Russian_Translation_v0.7.0.zip`** в разделе [**Релизы (Releases)**](https://github.com/memasevich/Gnomoria-Russian-Translation/releases).
3. Распакуйте все файлы и папку `Skin` в корневую папку игры (туда, где находится `Gnomoria.exe`).
4. Запустите файл **`Patcher.exe`**. Он выполнит инъекцию хука в `Gnomoria.exe` (оригинал будет сохранён как `Gnomoria.exe.backup`).
5. Запустите игру. Всё готово!
